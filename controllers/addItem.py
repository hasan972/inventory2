...
# create class
@action("addItem/item_class", method=["GET", "POST"])
@action.uses("addItem/addItem.html", auth, T, db)
def item_class():
    if not session.get('user_id'):
        ...
        if form.accepted:
            ...
            redirect(URL('addItem/item_class'))

    return dict(form=form, rows=rows, search_term=search_term, search_by=search_by, role=role, user=user, branch_name=branch_name)

# edit class
@action('addItem/edit_class/<class_id:int>',method=["GET","POST"])
@action.uses(db,session,flash,'addItem/edit_class.html')
...
# delete class
@action('addItem/delete_class/<class_id:int>',method=["GET","POST"])
